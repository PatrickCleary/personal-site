...
export const NavBar: React.FC<NavBarProps> = ({ refs, setClicked }) => {
  ...
  if(mobile)
  return (
    <div style={{width:'100%', justifyContent:'center',alignItems:'center', position:'fixed', top:0, display:'flex', zIndex: 49}}>
    <div
      className="Nav-Bar"
      style={{
        display: "flex",
    ...
    >
      {navBarOptions.map((option, index) => {
        return (
          ...
          >
            <p style={{ textAlign: "center", fontSize:'.80rem' }}>{option}</p>
          </div>
        );
      })}
    </div>
  ...
  return (
    <div style={{position:'fixed', top:0, right:0, display:'flex', zIndex: 49}}>
    <div
      className="Nav-Bar"
      style={{
        display: "flex",
