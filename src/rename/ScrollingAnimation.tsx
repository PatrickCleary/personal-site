...
export const ScrollingAnimation: React.FC<ScrollingAnimationProps> = ({
  position,
}) => {
  const { mobile, clientWidth } = useContext(WindowContext);
  const numTrains = mobile ? 4 : 5;
  ...
  for (let i = 0; i < numTrains; i++) {
    trains.push(
      <img
        className={`${
          mobile ? "Animation-Image-Mobile" : "Animation-Image-DT"
        }`}
        src={i === 0 ? LastTrain : Train}
  ...
  return (
    <div
      ...
      style={{
        left: `${position - clientWidth * scrollFactor}px`,
        display: "flex",
        justifyContent: "flex-end",
        width: mobile ? "125vw" : "110vw",
      }}
    >
      {trains}
    </div>
